    ...
    <div className={styles.main}>
      ...
      <h2>This is {JSON.stringify(summoners)}</h2> */}
        <Row className={styles.summoner_left}>
          {
            summoners.map((item: SummonerProps, index: number) => (
              <Col span={6} key={index} className={item.summoner_id === summonerItem?.summoner_id ? styles[`summoneritem-active`] : styles.summoneritem}>
                <img src={`https://game.gtimg.cn/images/yxzj/img201606/summoner/${item.summoner_id}.jpg`} alt={item.summoner_name} onClick={() => onClick(item)} />
        ...
        <div className={styles.summoner_right}>
          <img src={`https://game.gtimg.cn/images/yxzj/img201606/summoner/${summonerItem?.summoner_id}-big.jpg`} />
          <h4>{summonerItem?.summoner_name}</h4>
          <p>{summonerItem?.summoner_rank}</p>
          <div className={styles.summoner_desc}>{summonerItem?.summoner_description}</div>
        </div>
